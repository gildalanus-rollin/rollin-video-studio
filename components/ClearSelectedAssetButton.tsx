...
export default function ClearSelectedAssetButton({
  projectId,
  assetType,
}: Props) {
  ...
  const handleClear = async () => {
    ...
    const nextNotes = buildProjectNotes({
      ...
      selectedImage: assetType === "image" ? "" : parsed.selectedImage,
      selectedVideo: assetType === "video" ? "" : parsed.selectedVideo,
      selectedMusic: assetType === "music" ? "" : parsed.selectedMusic,
    ...
    if (updateError) {
      setClearing(false);
      setMessage(`Error al quitar la selección: ${updateError.message}`);
      return;
    }
