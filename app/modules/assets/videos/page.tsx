...
export default async function VideosAssetsPage({ searchParams }: PageProps) {
      ...
      <section className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div className="mt-5">
          ...
          <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-600">
            Esta sección intenta leer el bucket <strong>videos</strong> de Supabase
          ...
          {projectId ? (
            <div className="mt-4 inline-flex rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-xs font-medium text-emerald-700">
              seleccionando un video para este proyecto
            </div>
          ) : null}
        </div>
      </section>
      ...
      {error ? (
        <section className="rounded-[28px] border border-red-200 bg-red-50 p-6 shadow-sm">
          <p className="text-sm font-medium text-red-700">
            Error al leer el bucket videos: {error.message}
          </p>
        ...
        <section className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
          <p className="text-sm text-slate-600">
            El bucket <strong>videos</strong> respondió, pero no devolvió archivos.
          </p>
        </section>
      ) : (
        <section className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
